...
### P2-B Gemini ツール（73/85 実測・残 12 は全て外部ランタイム/依存要）
...
>
> **⚠️ 残 12 ツールは「LLMツール完全実装」ゴールの障壁＝いずれも外部ランタイム/新依存の判断が要る（自律実装不能）**:
> - **browser 系 10 本**（`browserInteractiveOpen/Click/Type/Wait/Close/Status` + `fetchDynamicPage` + `takePageScreenshot` + `browserFillCredential` + `searchWeb`）= **ヘッドレス chromium ランタイム**が必要。現行スリム Docker（59MB）は chromium を**意図的に排除**済み → 再同梱（イメージ肥大）か chromium サイドカーの**インフラ判断**が要る。
