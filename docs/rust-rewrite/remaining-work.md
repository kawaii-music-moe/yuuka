...
- 最終更新: 2026-07-14（**admin Web-API セッション**: 管理系 `/api/admin/*` 15 エンドポイントを新規 `yuuka-admin` クレートで実装完了 + 敵対的パリティレビュー[14 観点]で確定 0 バグ。全ゲート緑 — build ✅ / clippy -D ✅ / **test 412** ✅ / deny exit0 ✅・新規依存なし）
...
| `cargo test --workspace` | ✅ **412 passed / 0 failed**（2026-07-14 admin 15 エンドポイント + `destroy_all_for_user` で +10） |
...
| HTTP ルート被覆 | 74 / 152 パス ≒ **49%**（認証 7 + `/api/me` + 9 ドメイン CRUD + **管理系 15**〔2026-07-14 admin〕） |
| Gemini ツール被覆 | 27 / 87 native ≒ **31%**（動的 MCP 0。2026-07-14 に clipboard 3 本追加・timeline expense/task_done も tool から実行可能に） |
