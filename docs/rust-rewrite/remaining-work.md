...
- 最終更新: 2026-07-14（**P2-A/P2-B 4 増分セッション**: admin 15 + settings 6 Web-API（新規 `yuuka-admin`/`yuuka-settings`）+ **todo ツール 4 本 + finance ツール 5 本**を実装 + 敵対的パリティレビューで確定 0 HIGH。全ゲート緑 — build ✅ / clippy -D ✅ / **test 424** ✅ / deny exit0 ✅・新規依存なし。ルート 80/152・ツール 36/87） 
...
| Gemini ツール被覆 | 36 / 87 native ≒ **41%**（動的 MCP 0。2026-07-14 に todo 4 本 + finance 5 本〔getMonthlySummary/getCategoryBreakdown/getBudgetLimits/setBudgetLimit/deleteBudgetLimit〕追加） |
...
- [~] todo: 4/11 済（2026-07-14＝**addSubtask/updateTodo/updateTaskProgress/getTaskDetail**・既存 TodoRepo メソッドへ配線）。**残 7**: editTodoTags/listTasksByTag/listTodoTags/getTaskUsageGuide/applyTaskPriorities/organizeTaskPriorities/stopTodoRoutine（タグ集計・優先度整理・ルーチン停止 repo メソッド or LLM 側が必要）
- [~] finance: 5/~14 済（2026-07-14＝**getMonthlySummary/getCategoryBreakdown/getBudgetLimits/setBudgetLimit/deleteBudgetLimit**・既存 FinanceRepo へ配線。`monthly_category_breakdown` に etype 引数を追加）。**残**: planned payments 一式（listPlannedPayments/addPlannedPayment/settlePlannedPayment/cancelPlannedPayment＝repo は有るので次増分）/findSettlementCandidates/linkPlannedPaymentTodo/linkPlannedPaymentReminder（cross-domain）
