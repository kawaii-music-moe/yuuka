...
// 既定は Vite 出力先 dist/public（P5 完了でフロントはここに一本化）。
// STATIC_DIR env 分岐は任意の配信元を指す汎用フックとして残すが、旧 src/public は撤去済のため
// vanilla ロールバック用途は廃止（緊急時は deploy rollback＝前イメージへ切替に一本化）。
...
// PWA は独立してビルドされる。開発用に旧静的出力にもフォールバックする。
const PWA_PUBLIC_DIR = fs.existsSync(path.join(PUBLIC_DIR, "pwa"))
	? path.join(PUBLIC_DIR, "pwa")
	: path.resolve(process.cwd(), "src", "public", "pwa");
...
function serveStaticFile(req: http.IncomingMessage, res: http.ServerResponse) {
	...
	fs.stat(resolvedPath, (err, stats) => {
		...
		// Vite のハッシュ付きアセット（assets/ 配下・入れ子可 + 内容ハッシュ命名）は長期不変キャッシュ可。
		// それ以外（index.html/theme-init.js/manifest.json/sw.js/404.html 等）は no-cache 維持。
